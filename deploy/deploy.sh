...
#  • Python: the "fastapi_setup" conda env (CONDA_ENV=… to use another); verified on every run and only
#            modified when --install-deps is given
#
...
if [[ "$SRC_DIR" != "$APP_DIR" ]]; then
  ...
  rsync -a --delete --exclude .git --exclude node_modules --exclude dist --exclude __pycache__ \
...
if ((DO_BACKEND)); then
  if ((INSTALL_DEPS)); then
    log "Installing Python dependencies into the conda env $PYBIN"
    echo "   note: this environment may be shared with other services — they get these versions too"
    "$PYBIN/python" -m pip install -r "$APP_DIR/$BACKEND_DIR/requirements.txt" || die "pip could not install $BACKEND_DIR/requirements.txt (see the resolver output above)"
    ok "python deps installed into $PYBIN"
  fi
  ...
  if py_can_run "$PYBIN"; then
    ...
    if [[ -n "$MISSING" ]]; then
      die "the conda env $CONDA_ENV is missing: $MISSING
       bash deploy/deploy.sh --install-deps"
    fi
    ok "environment satisfies the application — nothing was installed"
  else
    echo "── error from 'import app.main' in $PYBIN ──"
    tail -n 12 "$ENVLOG" >&2
    die "the conda env $CONDA_ENV cannot run the application (see the error above). Install the dependencies:
       bash deploy/deploy.sh --install-deps
   or, by hand:
       conda activate $CONDA_ENV && pip install -r $BACKEND_DIR/requirements.txt"
  fi
fi
